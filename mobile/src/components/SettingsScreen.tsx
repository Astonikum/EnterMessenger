...
import { CURRENT_VERSION, GITHUB_REPO_URL, fetchLatestRelease, isNewerVersion, type PlatformRelease } from "../github-releases";
...
function UpdatesContent({ state, onCheck }: { state: UpdateState; onCheck: () => void }) {
  ...
  return <View style={styles.stack}>
    <Section title="Версия"><StatusRow label="Мобильное приложение" value={`v${CURRENT_VERSION}`} detail="Expo iOS · Android · web" /></Section>
    ...
    <Section title="Источник обновлений"><Text style={[styles.statusDetail, { color: themeColors.muted }]}>Для mobile публикуется отдельный GitHub Release. Приложение не устанавливает обновления автоматически — выберите APK или AAB из релиза.</Text><Pressable onPress={() => { void Linking.openURL(`${GITHUB_REPO_URL}/releases`); }} accessibilityRole="link" accessibilityLabel="Открыть все релизы Enter Messenger"><Text style={[styles.linkText, { color: themeColors.primary, marginTop: 10 }]}>Все релизы Enter Messenger</Text></Pressable></Section>
  </View>;
}
