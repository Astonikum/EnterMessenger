...
export type PendingMedia = { file: File };
...
export function MessageComposer({ onSend = () => undefined, error = "", uploadProgress = null, replyTo, editingMessage, onEdit = () => undefined, onCancelContext = () => undefined }: MessageComposerProps) {
  ...
  return (
    <form onSubmit={submit} className="message-composer px-5 py-4">
      ...
      {pendingMedia.length > 0 && <div className="mb-2 flex flex-wrap gap-2">{pendingMedia.map(({ file }, index) => <PendingMediaPreview key={`${file.name}-${index}`} file={file} onRemove={() => setPendingMedia((current) => current.filter((_, itemIndex) => itemIndex !== index))} />)}</div>}
      <div className="flex items-end gap-2 rounded-2xl border border-border bg-surface p-2 focus-within:border-primary/50">
        ...
        <Button type="button" size="icon" variant="ghost" title="Прикрепить файл" aria-label="Прикрепить файл" onClick={() => fileInputRef.current?.click()} disabled={Boolean(editingMessage) || uploadProgress !== null}><Icon name="attach_file" className="size-4" /></Button>
