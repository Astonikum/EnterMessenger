...
export function AppRail({ profiles = [], activeProfile, folders = [], activeFolder = "all", showProfile = false, onSelectProfile = () => undefined, onRemoveProfile = () => undefined, onAddProfile = () => undefined, onBack = () => undefined, onSelectFolder = () => undefined, onToggleProfile = () => undefined }: AppRailProps) {
  return (
    <aside className="app-rail flex flex-col border-r border-border/70 bg-surface/35">
      ...
      <div className="mt-auto flex flex-col items-center gap-2 p-3">
        ...
        <button type="button" className="rail-button" title="Настройки" aria-label="Настройки" onClick={() => undefined}><Icon name="settings" className="size-5" /></button>
      </div>
    </aside>
  );
}
