...
export function AppRail({ profiles = [], activeProfile, folders = [], activeFolder = "all", showProfile = false, showSettings = false, onSelectProfile = () => undefined, onRemoveProfile = () => undefined, onAddProfile = () => undefined, onBack = () => undefined, onSelectFolder = () => undefined, onToggleProfile = () => undefined, onToggleSettings = () => undefined }: AppRailProps) {
  return (
    <aside className="app-rail flex flex-col border-r border-border/70 bg-surface/35">
      ...
      <div className="flex flex-col items-center gap-2 px-3 pt-1">
        <button type="button" className={cn("rail-button", activeFolder === "all" && !showProfile && !showSettings && "rail-button-active")} title="Все чаты" aria-label="Все чаты" onClick={() => { onSelectFolder("all"); onBack(); }}><Icon name="chat" className="size-5" /></button>
        {folders.map((folder) => <button key={folder} type="button" className={cn("rail-button", activeFolder === folder && !showProfile && !showSettings && "rail-button-active")} title={folder} aria-label={folder} onClick={() => { onSelectFolder(folder); onBack(); }}><Icon name="folder" className="size-5" /></button>)}
      </div>
