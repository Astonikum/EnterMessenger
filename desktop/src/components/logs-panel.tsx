...
export function LogsPanel({ onClose }: Props) {
  ...
  return <section className="logs-workspace" aria-labelledby="logs-title">
    <header className="logs-panel-header"><div><h1 id="logs-title">Логи</h1><p>Диагностика синхронизации, отправки и соединения</p></div><div className="flex items-center gap-2"><Button variant="outline" size="sm" disabled={logs.length === 0} onClick={clearLogs}><Icon name="delete" className="size-4" />Очистить</Button><button type="button" className="icon-button" title="Закрыть логи" aria-label="Закрыть логи" onClick={onClose}><Icon name="close" className="size-4" /></button></div></header>
    <div className="logs-panel-body">
      <p className="logs-meta">Журнал устройства · локально · записей: {logs.length} · содержимое сообщений не записывается</p>
