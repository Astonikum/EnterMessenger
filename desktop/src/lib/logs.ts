...
function readStoredLogs() {
  try {
    ...
    return Array.isArray(value) ? value.filter(isLogEntry).map(migrateLegacyEntry).slice(-MAX_LOGS) : [];
  } catch {
    return [];
  }
}
...
export function loadLogs() {
  if (!loaded) { entries = [...readStoredLogs(), ...entries].filter((entry, index, all) => all.findIndex((item) => item.id === entry.id) === index).map(migrateLegacyEntry).slice(-MAX_LOGS); loaded = true; }
  return entries;
}
